...
if os.name == "nt":
    ...
    def get_thumbnail_as_base64(file_path, size=128):
        ...
        try:
                ...
                if hbitmap_handle:
                    try:
                        ...
                        bmp = Image.frombuffer(
                            ...
                            ctypes.string_at(
                                bitmap_info.bmBits,
                                bitmap_info.bmWidthBytes * bitmap_info.bmHeight,
                            ),
                            "raw",
                            "BGRX",
                            bitmap_info.bmWidthBytes,
                            -1,
                        )
            ...
            print(f"Error extracting native thumbnail for {os.path.basename(file_path)}: {exc}")
            return None, file_path
        finally:
            if factory_ptr:
                del factory_ptr
        ...
        if bmp_copy:
            try:
                ...
                print(f"Failed to convert thumbnail for '{os.path.basename(file_path)}': {exc}")
        return None, file_path
...
def get_thumbnails_in_batch(file_paths):
